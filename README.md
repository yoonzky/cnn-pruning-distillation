...
**Дистилляция.** Класс `Distiller` с температурой и двумя слагаемыми лосса; ученик — один слой LSTM плюс один Dense, картинка 32x32x3 разворачивается в последовательность. Перебраны тридцать конфигураций: LSTM из 512, 256, 128, 64, 32, 16 и Dense из 128, 64, 32, 16, 10.

Цель "ученик не хуже учителя" не достигнута: лучший результат — около 55,7 % против 85 % у учителя, часть конфигураций вырождается в предсказание одного класса. Это предел подхода на таких данных: рекуррентная сеть плохо читает изображение как последовательность строк.
